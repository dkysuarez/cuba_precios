...
Lee detalle_*.json (Revolico) + grupo_*.json (Facebook)
...
- Anuncios de tiendas/talleres → descartar
...
def cargar_facebook() -> list:
    """Carga archivos de Facebook desde el scraper (formato grupo_*.json)"""
    # Buscar archivos con el nuevo formato: grupo_*.json
    ...
    for arch in archivos:
        try:
            ...
            if isinstance(datos, list):
                for raw in datos:
                    ...
                    if p:
                        todos.append(p)
                        procesados += 1
            # Si es un solo post (diccionario)
            ...
            print(f"  ✅ {arch.name}: {procesados} posts procesados")
